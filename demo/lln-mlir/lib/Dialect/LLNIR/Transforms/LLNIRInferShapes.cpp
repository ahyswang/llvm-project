...
#include "llnmlir/Dialect/LLNIR/Utils/ShapeUtils.h"

namespace mlir {
namespace llnir {
#define GEN_PASS_DEF_LLNIRINFERSHAPESPASS
...
bool canBeRefined(Operation *user) {
  if (!user->getDialect())
    return false;
  return user->getDialect()->getTypeID() == TypeID::get<LLNIRDialect>() ||
...
void propagateShapesInRegion(Region &region, TypeModificationState &state) {
  Dialect *tosaDialect = region.getContext()->getLoadedDialect<LLNIRDialect>();
...
void validateSameOperandsAndResultRankTrait(Region &region) {
  ...
  for (auto &block : region) {
    for (auto &op : block) {
      if (!op.getDialect() ||
          op.getDialect()->getNamespace() != LLNIRDialect::getDialectNamespace())
        continue;
...
struct LLNIRInferShapes
    : public llnir::impl::LLNIRInferShapesPassBase<LLNIRInferShapes> {
public:
