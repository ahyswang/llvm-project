...
bool canBeRefined(Operation *user) {
  if (!user->getDialect())
    return false;
  return user->getDialect()->getTypeID() == TypeID::get<StandaloneDialect>() ||
...
void propagateShapesInRegion(Region &region, TypeModificationState &state) {
  Dialect *tosaDialect = region.getContext()->getLoadedDialect<StandaloneDialect>();
...
void validateSameOperandsAndResultRankTrait(Region &region) {
  ...
  for (auto &block : region) {
    for (auto &op : block) {
      if (!op.getDialect() ||
          op.getDialect()->getNamespace() != StandaloneDialect::getDialectNamespace())
        continue;
...
struct StandaloneInferShapes
    : public standalone::impl::StandaloneInferShapesPassBase<StandaloneInferShapes> {
public:
