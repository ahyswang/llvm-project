...
This is an example of an out-of-tree [MLIR](https://mlir.llvm.org/) dialect along with a llnir `opt`-like tool to operate on that dialect.
...
cmake --build . --target check-llnir
```
...
cmake -G Ninja `$LLVM_SRC_DIR/llvm` \
    ...
    -DLLVM_EXTERNAL_PROJECTS=llnir-dialect -DLLVM_EXTERNAL_LLNIR_DIALECT_SOURCE_DIR=../
cmake --build . --target check-llnir
```
